...
public class AddDoctorDialog extends JDialog {
	...
	public AddDoctorDialog(DoctorGUI doctorgui, DoctorDAO doctordao, Doctor prev, boolean updatemode) {
		this();
		doctorDAO = doctordao;
		doctorGUI = doctorgui;
	...
	private void populateGui(Doctor docs) {

		idTextField.setText(docs.getDocID());
		firstNameTextField.setText(docs.getDocFirstName());
		lastNameTextField.setText(docs.getDocLastName());
		sexTextField.setText(docs.getDocSex());
		phoneNumberTextField.setText(docs.getDocPhoneNumber());
		addressTextField.setText(docs.getDocAddress());
		departmentTextField.setText(docs.getDocDepartment());
	}

	public AddDoctorDialog(DoctorGUI doctorgui, DoctorDAO doctordao) {
		this(doctorgui, doctordao, null, false);
	}
	...
	protected void saveDoctorRecord() {
		...
		if (id.equals("") && firstName.equals("") && lastName.equals("") && sex.equals("") && phoneNumber.equals("")
			...
			setVisible(false);
			dispose();
			JOptionPane.showMessageDialog(doctorGUI, "Record is empty!", "Error", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		...
		if (updateMode) {
			...
			temp.setDocID(id);
			temp.setDocFirstName(firstName);
			temp.setDocLastName(lastName);
			temp.setDocSex(sex);
			temp.setDocPhoneNumber(phoneNumber);
			temp.setDocAddress(address);
			temp.setDocDepartment(department);

		} else {
			temp = new Doctor(id, firstName, lastName, sex, phoneNumber, address, department,null);
		}
		...
		try {
			...
			if (updateMode) {
				doctorDAO.updateRecord(temp);
			} else {
				doctorDAO.addRecord(temp);
			}
			...
			doctorGUI.refreshView();

			// Show success message

			if (updateMode) {
				JOptionPane.showMessageDialog(doctorGUI, "Record updated succesfully.", "",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(doctorGUI, "Record added succesfully.", "",
						JOptionPane.INFORMATION_MESSAGE);
			}
		} catch (Exception exc) {
			JOptionPane.showMessageDialog(doctorGUI, "Error adding record: " + exc.getMessage(), "Error",
