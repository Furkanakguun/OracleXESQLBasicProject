...
public class NurseGUI extends JFrame {
	...
	public NurseGUI() {
...

				{
					try {
						...
						if (id != null && id.trim().length() > 0) {
							...
							list = nurseDAO.searchID(id);

						} else if (firstName != null && firstName.trim().length() > 0) {

							list = nurseDAO.searchFN(firstName);

						} else if (lastName != null && lastName.trim().length() > 0) {

							list = nurseDAO.searchLN(lastName);

						} else if (sex != null && sex.trim().length() > 0) {

							list = nurseDAO.searchS(sex);

						} else if (phoneNumber != null && phoneNumber.trim().length() > 0) {

							list = nurseDAO.searchPN(phoneNumber);

						} else if (address != null && address.trim().length() > 0) {

							list = nurseDAO.searchA(address);

						} else if (department != null && department.trim().length() > 0) {

							list = nurseDAO.searchD(department);

						} else {

							list = nurseDAO.getAllRecords();
						}

						// Create the table view and update the main table

						NurseTableModel view = new NurseTableModel(list);
						...
						JOptionPane.showMessageDialog(NurseGUI.this, "Error: " + exc, "Error",
		...
		gbl_panel_1.columnWidths = new int[] { 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
				30, 30, 30, 30, 30, 0, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
				30, 30 };
			...
			public void actionPerformed(ActionEvent e) {
				{
					try {
						...
						if (row < 0) {
							JOptionPane.showMessageDialog(NurseGUI.this, "No record selected.", "Error",
									JOptionPane.ERROR_MESSAGE);
							return;
						}
						...
						int response = JOptionPane.showConfirmDialog(NurseGUI.this, "Delete this record?", "",
						...
						Nurse temp = (Nurse) table.getValueAt(row, NurseTableModel.OBJECT_COL);

						// Delete the selected record

						nurseDAO.deleteRecord(temp.getNurID());
						...
						JOptionPane.showMessageDialog(NurseGUI.this, "Record deleted succesfully.", "",
								JOptionPane.INFORMATION_MESSAGE);

					} catch (Exception exc) {
						JOptionPane.showMessageDialog(NurseGUI.this, "Error deleting record: " + exc.getMessage(),
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				AddNurseDialog dialog = new AddNurseDialog(NurseGUI.this, nurseDAO, temp, true);
			...
			public void actionPerformed(ActionEvent e) {
				{
					...
					AddNurseDialog dialog = new AddNurseDialog(NurseGUI.this, nurseDAO);
	...
	public void refreshView() {
		...
		try {
			List<Nurse> nurs = nurseDAO.getAllRecords();

			// create the model and update the "table"
			NurseTableModel view = new NurseTableModel(nurs);
